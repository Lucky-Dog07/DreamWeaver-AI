...
- **本地持久化**：所有作品自动保存到本地，刷新或重启后依然可见
- **多媒体展示**：支持绘画、音乐、视频的统一展示和播放
- **分页浏览**：优雅的网格布局，支持分页查看所有作品
- **详情查看**：查看作品的完整元数据、分析结果和情感标签
- **一键下载**：支持下载原画、音乐、视频到本地
- **存储管理**：实时显示存储路径、文件数量和占用空间
...
**数据存储**:
- 所有作品自动保存到 `data/artworks/default_user/` 目录
- 支持刷新页面和重启系统后数据依然可见
- 可在侧边栏查看存储路径和占用空间
